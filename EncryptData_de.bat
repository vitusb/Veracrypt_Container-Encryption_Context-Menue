...
# Powershell Encoding
# ####################################################################
# Das PS Skript muss eigentlich mit UTF-8 (BOM) kodiert werden, damit
# Umlaute bei Programmablauf korrekt dargestellt werden. Das hebelt
# aber die Inline-Erkennung von PS-Code im Batchfile aus; deswegen
# hier KEINE BOM-Kodierung.
# ####################################################################
# Workaround:
# https://stackoverflow.com/q/65305790/#comment115513164_65307397
# Zeichensatz in VBA (0 - 127)
# https://docs.microsoft.com/de-de/office/vba/language/reference/user-interface-help/character-set-0127
# Zeichensatz in VBA (128–255)
...
$host.UI.RawUI.WindowTitle = "Veracrypt Container-Verschl$([char]252)sselung";
...
Write-Host "# Veracrypt Container-Verschl$([char]252)sselung #" -ForegroundColor Yellow
Write-Host "#######################################" -ForegroundColor Yellow
Write-Host ""
...
if ([string]::IsNullOrEmpty($inputPath)) {
    Write-Host "###############################################################################"
    Write-Host "# ${IMySelf}: Mit VeraCrypt einen Ordner in einen Container verschl$([char]252)sseln !"
    Write-Host "###############################################################################"
    Write-Host "${IMySelf} ""Quell-Ordner"" ""Ziel-Conatainer"""
    Write-Host ""
...
# VeraCrypt absoluter Programm-Pfad
$veracryptPath = "C:\Program Files\VeraCrypt"
# VeraCrypt Hauptprogramm
$veraCryptExe = Join-Path $veracryptPath "VeraCrypt.exe"
# VeraCrypt Formatierprogramm
$veraCryptFormatExe = Join-Path $veracryptPath "VeraCrypt Format.exe"
# Konstanten zur Berechnung der Grösse des exFAT Dateisystems
$InitialVBRSize = 32KB
$BackupVBRSize = 32KB
$InitialFATSize = 128KB
$ClusterSize = 32KB      # TODO: Diesen Wert konfigurierbar machen
$UpCaseTableSize = 128KB # Typische Grösse

# Check if Veracrypt exists ...
if (-not (Test-Path -LiteralPath "$veraCryptExe")) {
    Write-Host "Das Programm `"Veracrypt`" existiert nicht ..." -ForegroundColor Red
    Write-Host "Bitte geben Sie in diesem Script einen g$([char]252)ltigen Pfad an" -ForegroundColor Red
    Write-Host "oder installieren Sie das Programm `"Veracrypt`" !!" -ForegroundColor Red
    Start-Sleep -s 6
    exit 1
}
...
if (-not (Test-Path -LiteralPath "$veraCryptFormatExe")) {
    Write-Host "Das Programm `"VeraCrypt Format`" existiert nicht ..." -ForegroundColor Red
    Write-Host "Bitte geben Sie in diesem Script einen g$([char]252)ltigen Pfad an" -ForegroundColor Red
    Write-Host "oder installieren Sie das Programm `"Veracrypt`" !!" -ForegroundColor Red
    Start-Sleep -s 6
    exit 1
...
if (-not (Test-Path $inputPath)) {
    Write-Host "Der angegebene Eingabepfad existiert nicht."
    Write-Host "Bitte geben Sie einen g$([char]252)ltigen Eingabepfad an."
    Start-Sleep -s 6
    exit 1
}
...
#: Write-Host "[DEBUG]: containerPath ist: $containerPath" -ForegroundColor Blue

# Check if container path already exists
if (Test-Path -LiteralPath $containerPath) {
    Write-Host "Der angegebene Containerpfad existiert bereits."
    Write-Host "Bitte geben Sie einen eindeutigen Pfad f$([char]252)r den neuen Container an."
    Start-Sleep -s 6
    exit 1
}
...
if (-not (Test-Path "$containerPathOF")) {
    Write-Host "Der angegebene Pfad des Conatainers existiert nicht !"
    Write-Host "Bitte geben Sie einen existierenden Pfad f$([char]252)r den neuen Container an."
    Start-Sleep -s 6
    exit 1
}
...
function Get-ExFATSizeRec {
    ...
    try {
        ...
        Write-Error "Fehler bei der Verarbeitung des Elements im Pfad ${Path}: $_"
    }
...
function Get-ExFATSize {
    ...
    try {
        ...
        Write-Error "Fehler bei der Berechnung der exFAT-Gr$([char]246)$([char]223)e f$([char]252)r den Pfad ${Path}: $_"
        return 0
    }
}
...
Write-Host ("Containergr$([char]246)sse: $containerSize MB") -ForegroundColor Yellow
...
#
$Password01  = Read-Host ("Bitte Passwort eingeben") -AsSecureString
$Password02  = Read-Host ("Bitte Passwort wiederholen") -AsSecureString
...
if ( $PWMatch ) {
    ...
    Write-Host ("")
    Write-Host -ForegroundColor Red ("Die beiden Passw$([char]246)rter stimmen nicht $([char]252)berein !")
    Write-Host -ForegroundColor Red ("Achten Sie auf GROSS und Kleinschreibung und")
    Write-Host -ForegroundColor Red ("vermeiden Sie Umlaute oder Sonderzeichen.")
    Write-Host -ForegroundColor Red ("Ich Breche den Vorgang ab !")
    Start-Sleep -s 6
    exit 1
}
...
Write-Host "Erzeuge VeraCrypt Container `"$containerPath`" ..."
...
if (-not (Test-Path -LiteralPath $containerPath)) {
    Write-Host "Beim Erstellen des VeraCrypt Containers ist ein Fehler aufgetreten."
    Start-Sleep -s 6
    exit 1
}
...
if ($null -eq $unusedDriveLetter) {
    ...
    Write-Error "Es wurden keine verf$([char]252)gbaren Laufwerksbuchstaben gefunden."
    Write-Error "Geben Sie bitte einen Laufwerksbuchstaben frei und versuchen Sie es erneut."
    Start-Sleep -s 6
    exit 1
}

Write-Host "Binde den neu erstellten VeraCrypt Container in das System ein ..."
...
if (-not (Test-Path -LiteralPath $mountedDriveRoot)) {
    # Volume mount failed
    Write-Error "Das Volumen bzw. Abbild konnte nicht eingebunden werden."
    Write-Error "Bitte stellen Sie sicher, dass das Programm VeraCrypt.exe fehlerfrei arbeitet."
    # delete the file container that was created
...
Write-Host "Kopiere Daten in den eingebundenen VeraCrypt Container ..."
...
Write-Host "Kopieren abgeschlossen. Der VeraCrypt Container wird vom System getrennt ..."
...
Write-Host "Der VeraCrypt Container wurde erfolgreich angelegt."
